...
@tf.function
def cross_entropy(predicted_labels, labels_one_hot):
    # TODO: zaimplementuj mnie, pieknie prosze!
    # predicted_labels - wektor przykładów (nie jeden przykład (dlatego jest [100,10] a nie [100,1])
    # labels_one_hot (labels w wersji one-hot, przetworzone w funkcji uczącej model)
    y = tf.math.log(predicted_labels) * labels_one_hot
...
def measure_loss_and_accuracy(dataset, batch_size, model):
    ...
    for images, labels in tqdm(dataset.batch(batch_size)):
        ...
        one_hot = tf.one_hot(labels, tf.shape(label_probabilities)[1], dtype='float32')  # ONE-HOT
        loss = cross_entropy(label_probabilities, one_hot)
...
class TestChapterOne(unittest.TestCase):
    ...
    def test_exercise_two(self):
        ...
        # wszystkie parametry czyli w1, b1, w2, b2
        self.evaluate_model(
            model=model,
    ...
    def test_exercise_three(self):
        ...
        w1 = tf.Variable(tf.random.truncated_normal([784, 100], 0.0, 0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
        b1 = tf.Variable(tf.constant(0.1, shape=[100]))  # TODO: zaimplementuj mnie, pieknie prosze!
        w2 = tf.Variable(tf.random.truncated_normal([100, 10], 0.0, 0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
        b2 = tf.Variable(tf.constant(0.1, shape=[10]))  # TODO: zaimplementuj mnie, pieknie prosze!
...
class TestChapterTwo(unittest.TestCase):
    ...
    def test_exercise_four(self):
        ...
        # czy musimy rzeczywiscie kontrolowac recznie kazdy tensor? nie! mozemy wykorzystac gotowe juz elementy!
        ...
        model.compile(
            ...
            loss=tf.keras.losses.BinaryCrossentropy(),  # TODO: zaimplementuj mnie, pieknie prosze!
            ...
            optimizer=tf.keras.optimizers.SGD(lr=0.7),  # TODO: zaimplementuj mnie, pieknie prosze!
...
class TestChapterThree(unittest.TestCase):
    def test_exercise_five(self):
        ...
        # powyzsza siec ma 19 warstw (block1 i block2 mają dwie warstwy, block3,4,5 mają ich po cztery - razem 16)
        # do tego jeszcze 3 dodatkowe - stąd nazwa vgg19
        ...
        # sieć radzi sobie dobrze póki są to niżej wymienione klasy oraz zdjęcie jest w miarę oczywiste
        # dla podchwytliwych zdjęć np. kota leżącego na kocu o podobnym kolorze co jego sierść sieć ma problem
        # przy zdjęciu człowieka również sieć miała problem - rozpoznała rzeczy typu marynarka itp. ale
        # człowieka jako tako nie rozpoznała - być może nie ma takiej klasy
        ...
        layer_to_preview = 12  # numer warstwy, ktorej aktywacje podgladamy
        ...
        # obraz zdaje się coraz bardziej zatracać
        # być może jest to równoznaczne z lepszym zrozumieniem danego obrazka
        # obraz wydaje się coraz bardziej falować np. dla warstwy 4 jest to już dość ciekawy widok
        # przy coraz większej warstwie obraz zdaje się pomniejszać i wyglądać jak mała tablica pikselów
